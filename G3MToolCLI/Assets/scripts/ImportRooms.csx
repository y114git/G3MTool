...
void UpdateRoomFromJson(GameMakerRoom room, JsonElement data)
{
    ...
    if (data.TryGetProperty("creationCodeId", out JsonElement codeIdElm) && codeIdElm.ValueKind == JsonValueKind.String)
    {
        ...
        if (!string.IsNullOrEmpty(codeName))
        {
            ...
            if (code == null)
            {
                ...
                code = new GameMakerCode();
                code.Name = Data.Strings.MakeString(codeName);
                Data.Code.Add(code);

                var codeLocals = new GameMakerCodeLocals();
    ...
    if (data.TryGetProperty("flags", out JsonElement flagsElm) && flagsElm.ValueKind == JsonValueKind.Number)
        room.Flags = (GameMakerRoom.RoomEntryFlags)flagsElm.GetInt32();
    ...
    if (data.TryGetProperty("backgrounds", out JsonElement backgroundsElm) && backgroundsElm.ValueKind == JsonValueKind.Array)
    {
        ...
        {
            var bg = new GameMakerRoom.Background();
            bg.ParentRoom = room;
    ...
    if (data.TryGetProperty("views", out JsonElement viewsElm) && viewsElm.ValueKind == JsonValueKind.Array)
    {
        ...
        {
            var view = new GameMakerRoom.View();
    ...
    if (data.TryGetProperty("gameObjects", out JsonElement gameObjectsElm) && gameObjectsElm.ValueKind == JsonValueKind.Array)
    {
        ...
        {
            var gameObj = new GameMakerRoom.GameObject();
    ...
    if (data.TryGetProperty("tiles", out JsonElement tilesElm) && tilesElm.ValueKind == JsonValueKind.Array)
    {
        ...
        {
            var tile = new GameMakerRoom.Tile();
    ...
    if (Data.IsGameMaker2() && data.TryGetProperty("layers", out JsonElement layersElm) && layersElm.ValueKind == JsonValueKind.Array)
    {
        ...
        foreach (JsonElement layerElm in layersElm.EnumerateArray())
        {
            ...
            var layer = new GameMakerRoom.Layer();
            int layerType = layerTypeElm.GetInt32();
            layer.LayerType = (GameMakerRoom.LayerType)layerType;
            ...
            if (layerType == (int)GameMakerRoom.LayerType.Instances)
            {
                var instancesData = new GameMakerRoom.Layer.LayerInstancesData();
            ...
            else if (layerType == (int)GameMakerRoom.LayerType.Tiles)
            {
                var tilesData = new GameMakerRoom.Layer.LayerTilesData();
            ...
            else if (layerType == (int)GameMakerRoom.LayerType.Background)
            {
                var bgData = new GameMakerRoom.Layer.LayerBackgroundData();
            ...
            else if (layerType == (int)GameMakerRoom.LayerType.Assets)
            {
                var assetsData = new GameMakerRoom.Layer.LayerAssetsData();
                assetsData.LegacyTiles = new GameMakerPointerList<GameMakerRoom.Tile>();
                assetsData.Sprites = new GameMakerPointerList<GameMakerRoom.SpriteInstance>();
                if (Data.IsVersionAtLeast(2, 3))
                {
                    assetsData.Sequences = new GameMakerPointerList<GameMakerRoom.SequenceInstance>();
                }
                ...
                if (layerElm.TryGetProperty("assetsData", out JsonElement assetsDataElm) && assetsDataElm.ValueKind == JsonValueKind.Object)
                {
                    ...
                    if (assetsDataElm.TryGetProperty("sprites", out JsonElement spritesElm) && spritesElm.ValueKind == JsonValueKind.Array)
                    {
                        ...
                        {
                            var sprInst = new GameMakerRoom.SpriteInstance();
            ...
            else if (layerType == (int)GameMakerRoom.LayerType.Effect)
            {
                var effectData = new GameMakerRoom.Layer.LayerEffectData();
        ...
        foreach (JsonElement seqElm in sequencesElm.EnumerateArray())
        {
            if (seqElm.ValueKind == JsonValueKind.String)
            {
                ...
                if (!string.IsNullOrEmpty(seqName))
                {
                    ...
                    if (seq != null)
                    {
                        var seqRef = new GameMakerResourceById<GameMakerSequence, GameMakerChunkSEQN>();
                        seqRef.Resource = seq;
    ...
    if (Data.IsVersionAtLeast(2024, 13) && data.TryGetProperty("instanceCreationOrderIDs", out JsonElement orderIdsElm) && orderIdsElm.ValueKind == JsonValueKind.Array)
    {
        if (room.InstanceCreationOrderIDs == null)
            room.InstanceCreationOrderIDs = new GameMakerRoom.InstanceIDList();
