...
public class UndertaleBackground : UndertaleNamedResource, IProjectAsset, INotifyPropertyChanged, IDisposable
{
    ...
    /// <summary>
    /// Backwards-compatible alias used by G3MTool's older export/import pipeline.
    /// </summary>
    public uint GMS2UnknownAlways2
    {
        get => GMS2TilesetVersion;
        set => GMS2TilesetVersion = value;
    }
    ...
    /// <summary>
    /// Backwards-compatible alias for older code paths that stored the exported sprite by raw index.
    /// </summary>
    public int GMS2ExportedSpriteIndex
    {
        get => _gms2ExportedSprite?.CachedId ?? 0;
        set
        {
            if (value == 0)
            {
                _gms2ExportedSprite = null;
            }
            else
            {
                (_gms2ExportedSprite ??= new()).CachedId = value;
                _gms2ExportedSprite.Resource = null;
            }
            PropertyChanged?.Invoke(this, new PropertyChangedEventArgs(nameof(GMS2ExportedSpriteIndex)));
        }
    }
